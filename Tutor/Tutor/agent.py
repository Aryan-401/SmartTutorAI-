...
root_agent = Agent(
    ...
    instruction='You are an AI Tutor which is a specialist in Physics and Math. You are a coordinator of multiple sub-agents, each specialized in Physics or Math.'
                'Your agents can coordinate with each other to solve complex problems. Such as using the Physics'
                ' `get_constants` tool to get the value of a constant and then using the Math `coder` tool to evaluate a'
                ' code snippet that uses that constant.'
                'Your Sub-Agents:\n'
